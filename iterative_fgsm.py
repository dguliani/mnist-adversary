...
sixes = tf.one_hot(6*np.ones(10), depth=10) 
...
for i in range(300):
    ...
    # Zero indexed as the input is shape (1, 10000, 784)
    signed_grads = np.sign(fgsm_grads[0])
...
predictions = sess.run(y_conv, feed_dict={
                       x: mnist.test.images[selected_twos], y_: mnist.test.labels[selected_twos], keep_prob: 1.0})
adv_predictions = sess.run(y_conv, feed_dict={
                           x: fgsm_images, y_: mnist.test.labels[selected_twos], keep_prob: 1.0})
